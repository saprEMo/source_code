...
print 'SHAPE_LC',SHAPE_LC,'SHAPE_ELC',SHAPE_ELC
...
while checkANS:
    ...
    if NinLC>0:
        ...
        if NinLC<SHAPE_LC[0]:
            ...
            print "np.shape(L_LC_Ared)",np.shape(L_LC_Ared)
...
if (Ans_HGabs=="yes" or Ans_HGabs=="y" or Ans_HGabs=="YES" or  Ans_HGabs=="Y"):
    ...
    print "np.shape(En_j_XLC)",np.shape(En_j_LC)
...
for el in range (0,len(list)):
    ...
    line = list[el]
    print 'line: ',line
    print "np.shape(line)",np.shape(line)
    dim_line = np.shape(line)
    print "dim_line",dim_line[0]
    if (dim_line[0]!=0):
        label = line[0]
    else:
        continue
